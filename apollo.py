...
def extractdatazip(mod_info,database_names):
    ...
    print(data_dir)
    with zipfile.ZipFile(str(data_dir), "r") as f:
        for name in f.namelist():
            tmp_name = os.path.basename(name)
            if tmp_name in database_names:
                for mod_def, mod_data in mod_info.items():
                    if mod_data:
                        if mod_data[1] == tmp_name:
                            print(mod_data[1])
                            f.extract(name)
                            temp_path = str(pathlib.Path().resolve())
                            print(temp_path + name)
                            mod_info[mod_def].append(temp_path + name)
...
def parse_module_definition(mod_info):
        ...
        for f in filenames:
            if f.endswith(".txt"):
                ...
                for db in database_name:
                    ...
                    if subparser == 'extract' or subparser == 'extractzip':
                        if version == 'yolo':
    ...
    elif subparser == 'extractzip':
        extractdatazip(mod_info,database_names)
...
if __name__ == "__main__":
    ...
    extract.add_argument('-v'
        , choices=['8','9','10','11','12','13','14','15','10.13','10.14','10.15','10.16', 'and9','and10','and11','win10_1803','win10_1809','win10_1903','win10_1909','yolo']
        , required=True
        , action="store"
        , help="OS Version (required). iOS=8-15, macOS=10.13-10.15, android 9-11, Windows 10 1803-1909")
    extract.add_argument('-k'
        , help="Additional KMZ Output for Location Data"
        , action="store_true")

    extractzip = subparsers.add_parser('extractzip'
        , help='Extract Data using APOLLO Modules')
    extractzip.add_argument('-o'
        , choices=['sql','csv','sql_json']
        , required=True
        , action="store"
        , help="Output: sql=SQLite or csv=CSV (required)")
    extractzip.add_argument('-p'
        , choices=['apple','android','windows','yolo']
        , default='apple'
        , action="store"
        , help="Platform: apple=iOS/macOS, Android, Windows or yolo (run on whatever")
    extractzip.add_argument('-v'
        , choices=['8','9','10','11','12','13','14','15','10.13','10.14','10.15','10.16', 'and9','and10','and11','win10_1803','win10_1809','win10_1903','win10_1909','yolo']
        , required=True
        , action="store"
        , help="OS Version (required). iOS=8-15, macOS=10.13-10.15, android 9-11, Windows 10 1803-1909")
    extractzip.add_argument('-k'
        , help="Additional KMZ Output for Location Data"
        , action="store_true")
